...
void setConfig(Config c) {
    ...
    hausdorff = c.exists("hausdorff") ? c.pDouble("hausdorff") : 0.006;//Wang
    layerThreshold = c.exists("num_layers") ? c.pDouble("num_layers") : (c.exists("lThreshold") ? c.pDouble("lThreshold") : 0);
    Thresholdtime = c.exists("Thresholdtime") ? c.pDouble("Thresholdtime") : 1;
    ...
    ADAPTIVE = c.exists("adaptive") ? (bool)(c.pInt("adaptive")) : true;
...
//vector<std::pair<int, skel_tree_t*>>* execute_skeleton_pipeline(FIELD<float>* im, int *firstLayer) {
  int execute_skeleton_pipeline(FIELD<float>* im, float thresholdTime) { 
    ...
    il->removeLayers();
    il->computeSkeletons(SKELETON_SALIENCY_THRESHOLD);
...
void execute_color_pipeline(IMAGE<float>* im) {
    ...
    if(c_space == COLORSPACE::YCC){
        ...
        if(Thresholdtime > 1) {
            SKELETON_SALIENCY_THRESHOLD += 0.3;
            //DistinguishableInterval -=2;
        }
...
///////////////
    ...
    if (!SMfield) {
        PRINT(MSG_ERROR, "Failed to read saliency map.\n");
        exit(EXIT_FAILURE);
    }
