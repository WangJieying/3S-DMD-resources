...
void getBinaryImage(FIELD<float>* f){
    auto f_dupe = f->dupe();
    ...
    for (int i = 0; i < f->dimX(); ++i) {
        for (int j = 0; j < f->dimY(); ++j) {
            ...
            int ox = outputFT[2 * id];
            int oy = outputFT[2 * id + 1];
            float dt_val = sqrt((i - ox) * (i - ox) + (j - oy) * (j - oy));
            //cout<<"    i "<<i<<"j "<<j<<"ox "<<ox<<"oy "<<oy;
            
            f->set(i,j,(dt_val > f_dupe->value(ox,oy) ? 0 : 255));
        }
    }
}
...
FIELD<float>* computeCUDADT(FIELD<float> *input, bool foreground) {
   ...
   //wang
    ...
    dt_to_field(input);
    //getBinaryImage(input);//uncomment this when using drawEachLayer function
