...
char * i; bool SaveMore = 0; int SuperResolution = 1;//wang
  ...
  auto begin = std::chrono::steady_clock::now();
        for (int inty : gray_levels) {
            ...
            if(interpolate){
                ...
                a_dm->getAlphaMapOfLayer(inty, Invert_current.at(InvertNum));
                //a_dm->drawEachLayer(inty, 0);
...
void draw_color_image(DataManager* dm_r, DataManager* dm_g, DataManager* dm_b) {
    ...
    cout << "Computing skeletons took = " <<  TotalTime << " （ms)" << endl;
     //Outfile1.close();
    //WWIDTH *= SuperResolution;
    //WHEIGHT *=SuperResolution;
    cout<<WWIDTH<<"!!"<<endl;
/**/
}
